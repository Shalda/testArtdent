...
<?php
	...
	function process_menu_item( $item ) {
		...
		if ( $this->nav_menu_item_exists( $item['post_id'] ) ) {
			return;
		}
		...
		if ( $id && ! is_wp_error( $id ) ) {
			update_post_meta( $id, '_menu_item_import_id', $item['post_id']);
	 ...
	 *
	 * Added by FlexiPress team
	*/
	function nav_menu_item_exists( $value ) {
		global $wpdb;

		$result = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->postmeta} WHERE meta_key = '_menu_item_import_id' AND meta_value = %s", $value ) );
