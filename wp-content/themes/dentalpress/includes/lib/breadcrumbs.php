...
function dentalpress_breadcrumbs() {

	/* === OPTIONS === */
	$text['home']     = esc_html__('Home', 'dentalpress'); // text for the 'Home' link
	$text['category'] = esc_html__("All posts from '%s' category", 'dentalpress'); // text for a category page
	$text['search']   = esc_html__('Search Results for "%s" Query', 'dentalpress'); // text for a search results page
	$text['tag']      = esc_html__("All posts tagged with '%s'", 'dentalpress'); // text for a tag page
	$text['author']   = esc_html__('Posts by %s', 'dentalpress'); // text for an author page
	$text['404']      = esc_html__('Error 404', 'dentalpress'); // text for the 404 page
	$text['page']     = esc_html__('Page %s', 'dentalpress'); // text 'Page N'
	$text['cpage']    = esc_html__('Comment Page %s', 'dentalpress'); // text 'Comment Page N'
	$text['shop']     = esc_html__('Products', 'dentalpress'); // text 'Shop'
	...
	if (is_home() || is_front_page()) {
		...
		// custom post type
			...
			if ( get_query_var('paged') ) {
				...
				if ($show_current == 1) echo ($delimiter) . $before . ((function_exists('is_shop') and is_shop()) ? $text['shop'] : $post_type->label) . $after;
			}
