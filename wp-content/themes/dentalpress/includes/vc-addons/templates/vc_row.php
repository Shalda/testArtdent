<?php 
	...
	if ( preg_match('~\bbackground(-image)?\s*:(.*?)\(\s*(\'|")?(?<image>.*?)\3?\s*\)~i', $atts['css'], $match) ) {
		...
		if ( !empty($atts['bg_image']) ) {
			$atts['bg_image'] = dentalpress_get_attachment_image_src($atts['bg_image'], 'full');
	...
	if ( $atts['enable_pattern'] == 'true' && !empty($atts['pattern_image']) ) {
		...
		$custom_css .= '.vu_row .'. esc_attr($custom_class) .'.vu_r-wrapper.vu_r-pattern-overlay:after{background-image:url('. esc_url(dentalpress_get_attachment_image_src($atts['pattern_image'], 'full')) .');'. (!empty($atts['pattern_opacity']) ? 'opacity:'. esc_attr($atts['pattern_opacity']) : '') .'}';
	}
